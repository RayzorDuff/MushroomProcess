...
DO $$ BEGIN
        IF NOT EXISTS (
          SELECT 1
          FROM pg_constraint c
          WHERE c.conname = 'uq_ecommerce_orders_clover_payment_id'
            AND c.conrelid = 'public.ecommerce_orders'::regclass
        ) THEN
          ALTER TABLE "public"."ecommerce_orders"
            ADD CONSTRAINT "uq_ecommerce_orders_clover_payment_id" UNIQUE ("clover_payment_id");
        END IF;
      END $$;
DO $$ BEGIN
        IF NOT EXISTS (
          SELECT 1
          FROM pg_constraint c
          WHERE c.conname = 'uq_ecommerce_orders_ecwid_event_id'
            AND c.conrelid = 'public.ecommerce_orders'::regclass
        ) THEN
          ALTER TABLE "public"."ecommerce_orders"
            ADD CONSTRAINT "uq_ecommerce_orders_ecwid_event_id" UNIQUE ("ecwid_event_id");
        END IF;
      END $$;
