/**
 * Auto-generated from airtable_interfaces/Interface_Print_Queue.txt
...
async function main() {
  console.log(`🔧 Upserting view: Print Queue`);
  const table = await getTable('print_queue');
  const meta = {
    fields: [],
    sort: [],
    allowExport: false,
  ...
  const view = await upsertView(table.id, 'Print Queue', meta);
  console.log(`✅ View ready: ${view.title || 'Print Queue'} (id=${view.id || 'unknown'})`);
