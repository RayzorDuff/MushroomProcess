/**
 * Auto-generated from airtable_interfaces/Interface_Package_Freeze_Dried.txt
 * Interface: Package Freeze Dried
 * Table: products
 *
...
async function main() {
  console.log(`🔧 Upserting view: Package Freeze Dried`);
  const table = await getTable('products');
  const meta = {
    fields: [],
    sort: [],
    allowExport: false,
  ...
  const view = await upsertView(table.id, 'Package Freeze Dried', meta);
  console.log(`✅ View ready: ${view.title || 'Package Freeze Dried'} (id=${view.id || 'unknown'})`);
