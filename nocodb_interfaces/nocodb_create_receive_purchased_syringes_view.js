/**
 * Auto-generated from airtable_interfaces/Interface_Receive_Purchased_Syringes.txt
 * Interface: Receive Purchased Syringes
 * Table: items
 *
...
async function main() {
  console.log(`🔧 Upserting view: Receive Purchased Syringes`);
  const table = await getTable('items');
  const meta = {
    fields: [
  "name"
],
  ...
  const view = await upsertView(table.id, 'Receive Purchased Syringes', meta);
  console.log(`✅ View ready: ${view.title || 'Receive Purchased Syringes'} (id=${view.id || 'unknown'})`);
