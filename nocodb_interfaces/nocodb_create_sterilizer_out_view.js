/**
 * Auto-generated from airtable_interfaces/Interface_Sterilizer_Out.txt
 * Interface: Sterilizer OUT
...
async function main() {
  console.log(`🔧 Upserting view: Sterilizer OUT`);
  const table = await getTable('sterilizer_runs');
  const meta = {
    fields: [],
    sort: [],
    allowExport: false,
  ...
  const view = await upsertView(table.id, 'Sterilizer OUT', meta);
  console.log(`✅ View ready: ${view.title || 'Sterilizer OUT'} (id=${view.id || 'unknown'})`);
