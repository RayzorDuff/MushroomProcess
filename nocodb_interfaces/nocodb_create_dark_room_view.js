/**
 * Auto-generated from airtable_interfaces/Interface_Dark_Room.txt
 * Interface: Dark Room
 * Table: lots
 *
...
async function main() {
  console.log(`🔧 Upserting view: Dark Room`);
  ...
  const view = await upsertView(table.id, 'Dark Room', meta);
  console.log(`✅ View ready: ${view.title || 'Dark Room'} (id=${view.id || 'unknown'})`);

  // Best-effort: create per-row actions (if your NocoDB supports view actions)
  const actions = [
  {
    "title": "Shake and Break",
    "updates": [
      {
        "column_name": "action",
        "value": "Shake"
  ...
  {
    "title": "Apply Diatomaceous Earth",
    "updates": [
      {
        "column_name": "action",
        "value": "ApplyDiatomaceousEarth"
      }
    ]
  },
  {
