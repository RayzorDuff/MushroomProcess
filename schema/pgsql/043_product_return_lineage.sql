-- 043_product_return_lineage.sql
...
WITH lot_edges AS (
  ...
  -- Issue #57: explicit Lot -> Product -> returned Lot bridge.  The direct
  -- lot edge is derived only from the two explicit relationships; no lineage
  -- is inferred from dates/items/strain values.
  SELECT
    'product_origin_lot'::text,
    j.lots_id,
    returned.nocopk
  FROM public.lots returned
  JOIN public._m2m_products_lots_origin_lots j
    ON j.products_id = returned.source_product_id
  WHERE returned.source_product_id IS NOT NULL
    AND j.lots_id <> returned.nocopk
),
lot_adjacency AS (
  SELECT
...
lot_rows AS (
  SELECT
    ...
    CASE
      ...
      WHEN a.direction = 'upstream' AND a.relationship_type = 'product_origin_lot' THEN 'Product origin lot'
      ...
      WHEN a.direction = 'downstream' AND a.relationship_type = 'product_origin_lot' THEN 'Returned from packaged product'
...
source_product_rows AS (
  SELECT
    root.lot_nocopk,
    root.lot_id,
    'upstream'::text AS direction,
    'source_product'::text AS relationship_type,
    'Returned from product'::text AS relationship_label,
    'product'::text AS related_entity_type,
    p.nocopk AS related_nocopk,
    p.product_id AS related_id,
    p.name_mat AS related_item_name,
    ...
    p.use_by AS related_product_use_by,
    loc.name AS related_product_location
  FROM public.v_reporting_lot_lifecycle root
  JOIN public.lots returned
    ON returned.nocopk = root.lot_nocopk
  JOIN public.products p
    ON p.nocopk = returned.source_product_id
  LEFT JOIN public.locations loc
    ON loc.nocopk = p.storage_location_id
  WHERE returned.source_product_id IS NOT NULL
)
...
FROM (
  ...
  UNION ALL
  SELECT * FROM source_product_rows
) x
ORDER BY
  x.lot_nocopk,
  x.direction,
...
COMMENT ON VIEW public.v_reporting_lot_lineage IS
  'Issue #87/Issue #57 explicit adjacent lineage for lot-rooted Reporting. Exposes lot relationships, resulting products, and explicit Lot -> Product -> returned Lot transitions without inferred links.';
