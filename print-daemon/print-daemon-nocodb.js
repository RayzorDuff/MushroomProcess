/** print-daemon-nocodb.js (NocoDB-backed print_queue, steri_sheet still via Airtable)
 * =============================================================================
...
/* ---------- Airtable (still used for steri runs & lots) ---------- */
...
/* ---------- NocoDB (used for print_queue table) ---------- */
...
const NOCODB_QUEUE_TABLE_ID = process.env.NOCODB_QUEUE_TABLE_ID || 'print_queue';

const NC = axios.create({
...
/* ---------- Queue helpers (NocoDB-backed print_queue) ---------- */

/**
 * Fetch records from NocoDB print_queue where print_status = 'Queued'.
...
async function fetchQueued(viewName) {
  ...
  const url = `/api/v2/tables/${encodeURIComponent(NOCODB_QUEUE_TABLE_ID)}/records`;
...
async function markStatus(id, status, errorMsg = null) {
  ...
  const url = `/api/v2/tables/${encodeURIComponent(NOCODB_QUEUE_TABLE_ID)}/records/${encodeURIComponent(
    id
  )}`;
...
async function fetchLotsForRun(runId) {
  ...
  const url = `lots?filterByFormula=${encodeURIComponent(
    formula
  )}&pageSize=100`;
...
job.target_printer="${jobPrinter}" env.STERI_SHEET_PRINTER="${envPrinter}"`
    ...
    // write PDF path back to NocoDB print_queue (instead of Airtable)
    try {
      const url = `/api/v2/tables/${encodeURIComponent(
        NOCODB_QUEUE_TABLE_ID
