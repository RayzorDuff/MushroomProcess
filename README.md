...
- **Automations** implemented by Airtable scripts during parity testing, PostgreSQL functions for internal operations, and n8n for external integrations (`airtable_automation/`, `nocodb_schema/pgsql/`, `n8n/`)
- **Interfaces / Views** for station operators (`airtable_interfaces/`, `appsmith/`)
...
2. **Automations**  
   ...
   - On PostgreSQL: Internal workflow automation is implemented by functions and triggers imported from `nocodb_schema/pgsql/`.
...
4. **Install PostgreSQL workflow functions**
   - Import the ordered SQL modules under `nocodb_schema/pgsql/`.
