...
function sqlHeader() { return `-- Generated by airtable_export_to_postgres_sql_v2_latest.js\n-- ${new Date().toISOString()}\n`; }
...
function isComputed(f) { return ['lookup','multipleLookupValues','rollup','formula'].includes(f.type); }
function tableById(schema, tid) { return (schema.tables || []).find(t => t.id === tid); }
function primaryFieldId(table) { return table.primaryFieldId || null; }
...
function main() {
  ...
  // FieldId maps per table
  const tableFieldById = new Map();
  ...
  // Physical column set per table (for computed compiler)
  const physicalColsByTableSlug = new Map();

  // Junctions created from multipleRecordLinks: {jn, a, b, aCol, bCol, linkFieldName, linkFieldSlug}
  const junctions = [];
  ...
  for (const t of tables) {
    ...
    if (BIGINT_PKS) {
      cols.push(`${ident('nocopk')} BIGSERIAL PRIMARY KEY`); physical.add('nocopk');
    } else {
      cols.push(`${ident('airtable_id')} text PRIMARY KEY`); physical.add('airtable_id');
    }
  ...
  for (const t of tables) {
    ...
    for (const f of (t.fields || [])) {
      ...
      junctions.push({ jn, a, b, aCol, bCol, linkFieldName: f.name, linkFieldSlug });
      ...
      // FK to b table (even in self-link, b table same as a; bCol distinct)
      linksSql += `ALTER TABLE ${ident(POSTGRES_SCHEMA)}.${ident(jn)}\n`;
  ...
  if (CREATE_VIEWS) {
    ...
    for (const t of tables) {
      ...
      // Unique aliases for link arrays: include link field slug
      const linkCols = [];
  ...
  // 004_computed_views.sql (physical-only lookups/rollups)
  {
    let csql = sqlHeader();
    csql += `BEGIN;\n`;
    ...
    for (const t of tables) {
      ...
      for (const f of computedFields) {
        ...
        if (!physicalCols.has(targetSlug)) {
          // target is not a physical stored column; keep view valid
          exprs.push(`'{}'::text[] AS ${outCol}`);
          continue;
        }
        ...
        if (f.type === 'rollup') {
          csql += `-- TODO(rollup): ${aName}.${f.name} (aggregation formula not exported); emitted as array_agg\n`;
        }

        const expr =
          `(SELECT COALESCE(` +
          `array_agg(btbl.${ident(targetSlug)}::text ORDER BY btbl.${ident(targetSlug)}::text) ` +
          `FILTER (WHERE btbl.${ident(targetSlug)} IS NOT NULL), ` +
          `'{}'::text[]) ` +
          `FROM ${ident(POSTGRES_SCHEMA)}.${ident(jn)} j ` +
          `JOIN ${ident(POSTGRES_SCHEMA)}.${ident(b)} btbl ON btbl.nocopk = j.${ident((a === b) ? `${b}1_id` : `${b}_id`)} ` +
  ...
  console.log(' - 004_computed_views.sql');
  if (AIRTABLE_EXPORT_DIR) console.log(' - 010_load.sql + csv/*.csv');
}
