...
// - omit: do NOT generate those junction tables at all (canonical scalar FK only)
const DERIVED_M2M_MODE = (process.env.DERIVED_M2M_MODE || 'keep').toString().trim().toLowerCase();
...
function pgSafeIdent(name, maxLen = 63) {
  const n = String(name);
  if (n.length <= maxLen) return n;
  // Keep names deterministic and collision-resistant; preserve a readable prefix.
  const h = crypto.createHash('sha1').update(n).digest('hex').slice(0, 10);
  const keep = Math.max(1, maxLen - (1 + h.length)); // "_" + hash
  return n.slice(0, keep) + '_' + h;
}

function pgSafeName(prefix, name, maxLen = 63) {
...
const OPERATOR_EMAIL_TABLES = new Set(['lots', 'products', 'sterilization_runs', 'events']);
...
function extractEmailAddress(value) {
  ...
  const plain = s.match(/\b([^\s<>@]+@[^\s<>@]+\.[^\s<>@]+)\b/);
  return plain ? plain[1].trim().toLowerCase() : '';
}

function buildOperatorEmailLookup(table) {
  const lookup = new Map();
  const operatorField = (table && table.fields || []).find(f => f && f.name === 'operator');
  const choices = operatorField && operatorField.options && Array.isArray(operatorField.options.choices)
    ? operatorField.options.choices
    : [];

  for (const choice of choices) {
    const label = String((choice && choice.name) || '').trim();
    const email = extractEmailAddress(label);
    if (!label || !email) continue;

    const displayName = label.replace(/<\s*[^<>]+\s*>/g, '').trim();
    const firstName = displayName.split(/\s+/).filter(Boolean)[0] || '';

    for (const key of [label, displayName, firstName, email]) {
      if (key) lookup.set(String(key).trim().toLowerCase(), email);
    }
  }
...
function normalizeOperatorForImport(tableName, fieldName, value, operatorEmailLookup) {
  if (fieldName !== 'operator') return value;
  if (!OPERATOR_EMAIL_TABLES.has(slug(tableName))) return value;
  if (value === null || value === undefined || value === '') return value;

  const raw = String(value).trim();
  const displayName = raw.replace(/<\s*[^<>]+\s*>/g, '').trim();
  for (const key of [raw, displayName]) {
    const normalizedKey = String(key || '').trim().toLowerCase();
    if (operatorEmailLookup && operatorEmailLookup.has(normalizedKey)) return operatorEmailLookup.get(normalizedKey);
  }

  const directEmail = extractEmailAddress(value);
...
function sqlHeader() {
  // Keep import logs readable by suppressing NOTICE-level output (e.g. "does not exist, skipping").
...
function m2mJoinName(aSlug, linkFieldSlug, bSlug) {
  return pgSafeIdent(`_m2m_${aSlug}_${bSlug}_${linkFieldSlug}`);
}
...
`;
    ...
    for (const fk of fkInfos) {
      ...
      const fkCName = pgSafeIdent(`fk_${tn}_${fk.colName}`);
      const rel = `${POSTGRES_SCHEMA}.${tn}`;
      const fkCNameSql = fkCName.replace(/'/g, "''");
      const idxName = pgSafeIdent(`ix_${tn}_${fk.colName}`);
...
`;
    ...
    //     * if it's name (text), create a non-unique index
  {
    ...
    const addUnique = (colName) => {
      const cname = pgSafeIdent(`uq_${tn}_${colName}`);
...
`;
    ...
    const iname = pgSafeIdent(`ix_${tn}_${colName}`);
...
for (const t of exportTables) {
  ...
  for (const t of tables) {
    ...
    for (const f of (t.fields || [])) {
      ...
      // If this is a prefers-single link, we can omit the junction entirely (canonical FK only).
      if (prefersSingle && fkColName && DERIVED_M2M_MODE === 'omit') {
        // Still record metadata so vc_* generation can prefer FK joins.
        junctions.push({ jn, a, b, aCol, bCol, linkFieldSlug, linkFieldId: f.id, prefersSingle, fkColName, omitted: true });
        continue;
      }

      junctions.push({ jn, a, b, aCol, bCol, linkFieldSlug, linkFieldId: f.id, prefersSingle, fkColName, omitted: false });

      // Banner comment: helps distinguish true M2M from junctions retained/derived for prefers-single links.
      if (prefersSingle && fkColName) {
        linksSql += `\n-- DERIVED_M2M (prefersSingleRecordLink): canonical FK ${a}.${fkColName} -> ${b}.nocopk; junction retained for compatibility/auditing\n`;
      } else {
        linksSql += `\n-- M2M_LINK: Airtable multi-link field ${a}.${linkFieldSlug} -> ${b}\n`;
      }
  ...
  DEFERRABLE INITIALLY DEFERRED;\n`;
      ...
      if (prefersSingle && fkColName) {
        ...
        linksSql += `CREATE UNIQUE INDEX IF NOT EXISTS ${ident(pgSafeIdent(jn + '_' + aCol + '_uniq'))} ON ${ident(POSTGRES_SCHEMA)}.${ident(jn)}(${ident(aCol)});\n`;

        const fnA = pgSafeIdent(`sync_${a}_${fkColName}_to_${jn}`);
        const trgA = pgSafeIdent(`trg_${a}_${fkColName}_to_${jn}`);
        linksSql += `
...
`;
  ...
  if (CREATE_VIEWS) {
    ...
    for (const t of exportTables) {
      ...
      for (const j of junctions.filter(x => x.a === a)) {
        if (j.prefersSingle) continue; // prefer-single links are represented by scalar FK columns
  ...
  if (AIRTABLE_EXPORT_DIR && fs.existsSync(AIRTABLE_EXPORT_DIR)) {
    ...
    for (const t of exportTables) {
      ...
      const operatorEmailLookup = buildOperatorEmailLookup(t);
      ...
      const rows = rowsRaw.map(r => {
        ...
        for (const f of storedFields) {
          ...
          v = normalizeOperatorForImport(tableName, f.name, v, operatorEmailLookup);
